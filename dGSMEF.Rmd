...
Table 1 Ordinary differential equations and variales nomenclature 
...
Figure 3. HNC47 maximum aerobic glucose uptake rate, determined from batch experiments as the growth rate (0.69 h-1 HNC47) divided by the biomass yield (0.547g DW/g glucose).
...
![biomass_composition](biomass_composition.jpg)
...
$$ 1 - \Phi_Q  = \rho\times𝑟_{𝑚𝑎𝑥} $$

we could have

$$ frac{\lamda(\Phi_U)}{\lamda(\Phi_U = 0)} = 1 - frac{\Phi_U}{\Phi_C} $$

where $\Phi_C = \rho\times(r_{max} - r_0) \approx 0.48$ , $\lambda(\Phi_U)$ is the growth rate with $\Phi_U$ in the proteome, and $\lambda(\Phi_U = 0) is the growth rate without unnecessary protein in the proteome.
