...
```{r plotFigure,fig.height = 6, fig.width = 6}
...
## to compare the flux distributions along with fermetation time
...
library(ggplot2)
p = ggplot(pr_matrix,aes(x=PC1,y=PC2)) + geom_point(shape=23,aes(fill=Time)) + scale_fill_continuous(low="blue",high="red") 
#p + geom_text(aes(label=aa),size=4,hjust=0,vjust=-1,alpha=0.4) + guides(fill=guide_legend(title="hrs"))
